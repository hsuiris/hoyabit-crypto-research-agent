...
_CLAIM_PROMPT = (
    "你是加密貨幣研究分析師。只能使用下面提供的 Evidence，所有文字欄位一律使用繁體中文"
    "（幣種代號、指標名稱與 Evidence ID 保留原文）。\n\n"
    "**statement 必須是對下面 question 的直接回答**，寫成一句可以被證據檢驗的斷言。\n"
    "不要把題目換成另一個你更想回答的問題：\n"
    "- 題目問「現況、驅動因素與風險」→ statement 就講現況、驅動因素或風險，"
    "**不可**改寫成「價格將上漲／下跌」這種題目沒有要求的價格預測。\n"
    "- 題目問「兩個標的的差異」→ statement 要講差異本身。\n"
    "- 只有在題目本身要求驗證某個看法時（例如「市場認為 X，請找支持與反對證據」），"
    "才用那個看法當 statement，並填對應的 hypothesis_id。\n"
    "若題目包含多個子問題，就拆成多個 Claim，每個 Claim 回答一個子問題。\n\n"
