...
```bash
python -m unittest discover -s tests    # 全部測試（T7 後應為 534 passed；T0 基線為 117）
...
**配額警告**：免費方案每日 20 次呼叫，自 T4 起每次分析消耗 **4 次**（結構化推理、消息面摘要、
Critic 稽核、Claim 提案），因此每天約只能跑 5 次完整分析。若分析回應本身帶回 `claims`
（`src/llm.py` 的 `claim_proposals`），Claim 提案會直接沿用該回應，不再多打一次，維持 3 次。
測試前端改動時請用離線模式（`live=False, use_llm=False`），把真實呼叫留給最終驗證。
